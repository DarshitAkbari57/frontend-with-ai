...
const LazyActivityTicketModal = dynamic(
  () => import('./ActivityTicketModal').then((module) => module.ActivityTicketModal),
  { ssr: false }
);

export function ActivityTicketsSection({ activityId }: { activityId: number }) {
  ...
  useEffect(() => {
    async function fetchTickets() {
      try {
        const res = await fetch(`/api/activity/tickets/${activityId}`);
